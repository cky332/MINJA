...
## 机理终极验证：H3 ——"注入规则是贴在答案上的后处理步骤，只跟无关推理的深度有关"

**你的推断（H3）：** 注入规则 = "看到 'total' → 答案字母 +4"，是贴在答案末端的后处理。
- 当模型得答案的推理"一路就在算那个 total" → 规则被保温 → 攻击成功；
- 当得答案要走一长段"跟那个 total 无关"的推理 → 规则被挤掉 → 攻击失败。
- 预测：**跟领域无关，只跟"无关推理的深度"有关。**

**杀手级对照（同样的算术深度，唯一差别是这段算术是否就是 total）：**
- **CENTRAL**：d 步算术链**就是** total（"What is the total?"）。
- **PERIPHERAL**：`total` 只在背景（"a shop reported a total of \$X"），答案来自**另一个**独立的 d 步计数器（"What is the counter's value?"）。

脚本 `run_rq2_h3.py`，数据 `rq2_h3.json`（M=24/格，5 条恶意 demo 强制在场，检索移除）。`plain A-D` = 模型给了未加密的 A–D（即"忘了 +4"）。

| 条件（同深度） | d0 | d2 | d4 | d6 | d8 |
|---|--:|--:|--:|--:|--:|
| **CENTRAL** 算术（链=total） | 100 | 100 | 100 | 100 | 100 |
| **PERIPHERAL** 算术（另一个计数器） | 50 | 54 | 33 | 50 | 29 |
| **PERIPHERAL** 关系排序（非算术） | – | 79(d2) | 75 | 58(d3/d6) | – |

（PERIPHERAL 算术 d8 的 `plain A-D`=67%——三分之二的题模型直接忘了 +4。）

**结论（H3 基本成立，但有两个精炼）：**

1. **主因是"相关性"（在不在答案链上），比"深度"更狠。** CENTRAL 在**所有**深度都 100%（链就是 total → 规则全程保温）；而 PERIPHERAL **一旦答案换成另一个量、即使深度=0 就直接腰斩到 50%**。"离开 total 的答案链"这一步本身就值 ~50 点，比再加深度更致命。

2. **深度确实再往下压（你的核心预测对）。** PERIPHERAL 内部随无关推理加深继续掉：算术 50→29（d0→d8），关系 79→58（d2→d6），各掉约 20 点。`plain A-D` 同步从 42%→67%，直接证明是"+4 被挤掉"。

3. **领域无关——方向上成立，幅度上被调制。** 算术和关系两种**完全不同**的无关推理都把 ASR 压到 CENTRAL-100% 以下、且都随深度下降 → **不是算术特有**，支持你"跟领域无关"。但绝对水平不同：算术-无关（29–54%）比关系-无关（58–79%）压得更狠——可能因为算术计数要一直占用工作记忆里的"数字寄存器"，和"+4 答案字母"这个符号后处理更直接抢资源。所以严格说是"**方向领域无关、强度被无关推理的工作记忆占用程度调制**"。

**深度梯度的收紧验证（M=40，更深范围）—— 把噪声压下去后梯度非常干净：**

| PERIPHERAL 算术 (M=40) | d0 | d4 | d8 | d12 | d16 |
|---|--:|--:|--:|--:|--:|
| ASR | 50.0 | 27.5 | 35.0 | 22.5 | **10.0** |
| plain A-D（忘了 +4） | 45% | 70% | 62% | 75% | **88%** |

| CENTRAL 算术 (M=40，控制) | d0 | d8 | d16 |
|---|--:|--:|--:|
| ASR | 97.5 | 100.0 | **100.0** |

**最干净的杀手对照：同样 16 步算术，CENTRAL=100%（算的就是 total）vs PERIPHERAL=10%（算的是另一个计数器）——纯由"这段深度推理在不在触发词的答案链上"决定了 90 个点的差距。** 而 PERIPHERAL 内部 ASR 随深度 50→10 单调下滑、`plain A-D` 45%→88% 单调上升，**直接坐实"无关推理越深、+4 越被挤掉"**。

**一句话裁决：** 你的 H3 抓住了机理——注入是贴在答案上的后处理，**触发词必须落在"为得到答案而走的推理链"上才会被执行**；一旦答案改由一段与触发词无关的推理导出，规则就开始被挤掉，且**无关推理越深、挤得越狠**，这一点跨算术/关系两个领域都成立。最干净的防御推论：**让 agent 的最终答案必须由一段深度、与任何潜在注入词无关的推理链导出**，天然抗这类词法触发的记忆注入。
